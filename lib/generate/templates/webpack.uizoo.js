...
const reactConfigMaker = require('uizoo-app').reactConfigMaker;
const {
    serverPort,
    serverHost,
    serverProtocol,
...
class GenerateUiZooConfigsPlugin {
    apply(compiler) {
        compiler.plugin('watch-run', (c, cb) => {
            reactConfigMaker.createConfigs(componentsGlob, componentsRootDir, {ignoreTag})
                .then(() => cb())
...
server.listen(serverPort, 'localhost', () => {
    console.log(`    ${grey(`+*+*+`)} ${cyan(`UiZoo up on localhost:${serverPort}`)} ${grey(`+*+*+`)}`);
});