 ...
 *
 * Note: the ".bibliotheca-tooltip-wrapper" element is "display:inline-block" by default,
...
export default class Tooltip extends React.Component {
    ...
    render() {
        ...
        const tooltip = this.state.showTooltip && this.props.tooltip ? (
            <div className={`bibliotheca-tooltip-spacing-wrapper side-${tooltipSide} alignment-${tooltipAlignment}${extraClasses}`}>
                <div className='bibliotheca-tooltip-body'>{this.props.tooltip}</div>
            </div>) : null;
        const eventHandlers = this.getEventHandlers();

        return ( // tab index is needed for the onBlur event to work
            <div className="bibliotheca-tooltip-wrapper" {...eventHandlers} tabIndex="0">
                {tooltip}
                {this.props.children}
            </div>
        );
